...
mcp-preactivate: switch_project, search_codebase, find_symbol, get_function_source, get_full_context, get_dependencies
model: pi/smol
thinking-level: med
...
<directives>
- You MUST prefer the pre-activated Token Savior MCP tools over native equivalents for code navigation (e.g. prefer `search_codebase` over `search`, `get_full_context` over `read`, `find_symbol` over `find`). Call `switch_project` first to activate the project and trigger reindexing, then use the pre-activated MCP tools.
