...
- Fixed server-side tool-call parse errors (e.g. llama.cpp's "Failed to parse input at pos N") killing subagents instead of retrying. When the inference server cannot parse the model's generated tool-call syntax, the session now removes the failed turn, appends a developer reminder directing the model to re-emit with the correct structured format, and retries up to 3 times before giving up. Mirrors the existing empty-stop recovery pattern.
