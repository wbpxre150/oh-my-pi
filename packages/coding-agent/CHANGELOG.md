...
- Fixed DAP debugger silently reporting Kotlin breakpoints as "verified" when they will never fire due to JVM version mismatch (JDT-LS on JDK 21+ vs ART Java 8). The debugger now detects the mismatch warning from adapter output and appends an actionable warning when setting breakpoints on `.kt` files: breakpoints may not fire, use pause + step or `adb logcat` for Kotlin debugging.
- Fixed DAP `evaluate` on threads suspended via `pause` failing with an unhelpful "Thread must be suspended by step or breakpoint" error. The error is now augmented with the current suspension reason and guidance to use `step_in`/`step_over`. Successful evaluations under pause suspension now append a pre-warn that method invocations may fail.
...
- Fixed server-side tool-call parse errors (e.g. llama.cpp's "Failed to parse input at pos N") killing subagents instead of retrying. The retry now happens inline inside the agent loop's `runLoopBody` (mirroring the `XmlToolCallLeakInterruption` pattern) rather than at the `AgentSession` level via a scheduled post-prompt continue. The loop removes the failed turn, appends a steering reminder, and retries up to 3 times before giving up. The redundant `#handleParseError` / `#parseErrorRetryReminder` / `#parseErrorRetryCount` machinery and the `parse-error-retry.md` template have been removed from `AgentSession`.
...
- Fixed approved plans dumping every stage file's full body inline into the post-approval execution prompt and the rebuilt plan-reference message. Both `prompts/system/plan-mode-approved.md` and `prompts/system/plan-mode-reference.md` now list only the `local://stage-N.md` paths and instruct the agent to read each file itself, so the execution agent no longer receives one large concatenated document (which it would treat as authoritative and needlessly rewrite the on-disk stage files before handing off). The stage files remain the single source of truth on disk.
...
- Fixed `pause` not preparing the VM for evaluation. `pause()` now calls `#fetchTopFrame` after the stopped event to populate `session.stop.frameId`, so `evaluate` and `step_over` work after a pause instead of failing with "thread is not suspended."
- Fixed the `continued` event handler unconditionally clearing stop state for any thread's continued event. It now only clears when `allThreadsContinued` is true or the continued thread matches the stopped thread, preventing a worker thread's resume from clobbering another thread's stop state in multi-threaded Android apps.
...
- Fixed the Android (Kotlin) DAP auto-attach flow crashing with `null is not an object (evaluating 'client.proc.exited')`. TCP debug sessions (JDT-LS java-debug) construct the `DapClient` with `proc = null`, so `#registerSession` threw when it accessed `client.proc.exited`, leaving the session stuck in `launching`. The access is now null-safe (`client.proc?.exited`) and the liveness heartbeat self-clears via `clearInterval` once `isAlive()` returns false, so TCP sessions terminate cleanly on socket close without relying on a child-process exit signal.
- Fixed `evaluate` (and expression evaluation) failing with `Cannot evaluate, please specify projectName in launch.json` during Android (Kotlin) debug attach. The java-debug adapter requires `projectName` (= the Gradle module directory name). The Gradle module lookup is generalized beyond the hardcoded `app/` path to find whichever module declares the `applicationId`, and the module directory name is threaded through `AndroidAttachTarget` into the attach `extraAttachArguments`.
