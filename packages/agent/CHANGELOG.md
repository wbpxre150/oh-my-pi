...
- Fixed text tool-call fallback mode being reset to `false` after a successful turn. Previously, after the fallback activated and the model succeeded once, `textToolCallFallback` was cleared which sent the next turn back to structured tool-calling mode. Combined with `fallbackAttemptCount` not being reset, any subsequent parse-error cycle would exhaust retries with no fallback available, killing the subagent. The fallback now stays active for the rest of the session once triggered, and `fallbackAttemptCount` is reset on success so defensive re-entry is possible.
...
- The agent-loop XML-leak backstop now also scans finalized assistant **text** content (not just tool-call argument values) for leaked XML tool-call markup (`<antml:invoke>`, `<function=`, `<parameter=`) and server-side parse-error strings (e.g. llama.cpp's `Failed to parse input at pos N`). Bare `<function=NAME>` leaks that landed in visible text previously bypassed the backstop and never triggered a steering retry; they now engage the same `XmlToolCallLeakInterruption` recovery path. Gated on `localInferenceControl` as before.
